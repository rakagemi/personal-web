...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-60 mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full px-4 py-2 lg:flex border transition-all duration-100",
        visible ? "bg-white/10 dark:bg-neutral-900/40 border-black/10 dark:border-white/10" : "bg-transparent border-transparent",
        className,
      )}
    >
      {children}
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between px-0 py-2 lg:hidden border transition-all duration-300",
        visible
